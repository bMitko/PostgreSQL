...
SELECT 
	m.title,
	COUNT(r.movie_id) as review_per_movie
FROM movies m
...
SELECT 
	...
	COUNT(EXTRACT(YEAR FROM m.release_date)) as release_by_year,
	AVG(m.budget) as avg_bugdet
FROM movies m
GROUP BY m.movie_id, m.release_date
HAVING COUNT(EXTRACT(YEAR FROM m.release_date)) > 3 AND AVG(m.budget) > 50000000
...
SELECT 
	a.first_name,
	a.last_name,
	COUNT(a.actor_id) as lead_role_per_actor,
	SUM(mr.domestic_revenue + mr.international_revenue) as total_revenue
FROM actors a
JOIN cast_members cm ON a.actor_id = cm.actor_id
JOIN movies m ON cm.movie_id = m.movie_id
...
GROUP BY a.first_name, a.last_name
HAVING COUNT(a.actor_id) > 2 AND SUM(mr.domestic_revenue + mr.international_revenue) > 200000000
...
CREATE VIEW top_rated_movies 
AS
SELECT 
	m.title,
	TRUNC(AVG(r.rating), 1) as avg_rating,
	COUNT(r.movie_id) as review_per_movie,
	d.first_name || ' '|| d.last_name as director_fullname
FROM directors d
JOIN movies m ON d.director_id = m.director_id
JOIN reviews r ON m.movie_id = r.movie_id
GROUP BY m.title, m.movie_id, d.first_name, d.last_name
...
SELECT
	...
	(((mr.domestic_revenue + mr.international_revenue) - m.budget) - m.budget) as roi
FROM movies m
JOIN movie_revenues mr ON m.movie_id = mr.movie_id
GROUP BY m.movie_id, m.title, m.budget, mr.domestic_revenue, mr.international_revenue
ORDER BY m.movie_id ASC
...
SELECT 
	g.name,
	COUNT(DISTINCT m.movie_id) as movies_per_genre,
	TRUNC(AVG(r.rating), 2 )as avg_rating,
	SUM(mr.domestic_revenue + mr.international_revenue) as total_revenue
FROM genres g
FULL OUTER JOIN movie_genres mg ON g.genre_id = mg.genre_id
FULL OUTER JOIN movies m ON mg.movie_id = m.movie_id
FULL OUTER JOIN reviews r ON m.movie_id = r.movie_id
FULL OUTER JOIN movie_revenues mr ON m.movie_id = mr.movie_id
...
SELECT 
	pc.name,
	COUNT(mpc.movie_id) as movies_per_company,
	SUM(mpc.investment_amount) as total_investment,
	SUM(mr.domestic_revenue + mr.international_revenue) as total_revenue
FROM production_companies pc
FULL OUTER JOIN movie_production_companies mpc ON pc.company_id = mpc.company_id
FULL OUTER JOIN movies m ON mpc.movie_id = m.movie_id
FULL OUTER JOIN movie_revenues mr ON m.movie_id = mr.movie_id
GROUP BY pc.name, pc.company_id
ORDER BY pc.company_id ASC
